...
void WebotsController::SetMotorValues(char wheels[]) {
    for (int i = 0; i < 10; i++) {
...
void WebotsController::initdetect() {
    DistanceSensor* ds[3];
    char dsNames[3][5] = {
      "ds0", "ds2", "ds13"
    };
...
void WebotsController::SetWheelVelocity(double right, double left) {
    for (int i = 0; i < 10; i++) {
        if (i < 5)
